...
codestream_dump codestream::create(const opcode& code, u64 index) {
    return codestream_dump(*this, code, index);
}

void codestream::dump(std::ostream& out, const codestream_dump& cd) const {
    using std::setw;
    using std::setfill;
    using std::hex;
    using std::dec;

    const auto& code = cd.get_code();
    const auto op = code.op;
    const auto num = code.num;
    const auto index = cd.get_index();
...
std::ostream& operator<<(std::ostream& out, const codestream_dump& cd) {
    cd.get_codestream().dump(out, cd);
    return out;
}
